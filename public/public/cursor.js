...
const URLHash = CryptoJS.SHA1(window.location.href); 
// Create a unique user ID to differentiate between users             
const userID = push(ref(database, URLHash + '/users/')).key;
...
function generateUser(){
    set(ref(database, URLHash + '/users/' + userID), {
        cursorCordX: 0,
        cursorCordY: 0,
...
function updateCursorCordinates(event){
    update(ref(database, URLHash + '/users/' + userID), {
...
function updateOtherUsers(){
    onValue(ref(database, URLHash + '/users/'), (snapshot) => {
...
function removeInactiveCursors(){
    get(ref(database, URLHash + '/users/')).then((snapshot) => {
        for (var [key, value] of Object.entries(snapshot.val()) ){
